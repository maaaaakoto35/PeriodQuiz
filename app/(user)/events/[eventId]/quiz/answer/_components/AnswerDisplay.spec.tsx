...
vi.mock('../../_components', () => ({
  ...
  ChoiceButtonGroup: ({ choices, userSelectedChoiceId, showCorrectness }: any) => (
...
describe('AnswerDisplay', () => {
  ...
  describe('レンダリング', () => {
    ...
    it('背景画像が設定される', () => {
      ...
      expect(background?.style.backgroundImage).toContain('quiz_background.jpeg');
    });
  });
  ...
  describe('問題画像', () => {
    it('問題画像のURLが渡されると、QuestionContentに渡される', () => {
      ...
      expect(questionContent.querySelector(`img[src="${imageUrl}"]`)).toBeTruthy();
    });
  });
