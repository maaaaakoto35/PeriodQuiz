...
export function useRealtimeUserCount({
  ...
  const refetchUserCount = useCallback(async () => {
    try {
      ...
      console.error('[useRealtimeUserCount] Error fetching user count:', err);
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .subscribe((status) => {
        if (status === 'CLOSED') {
          console.warn('[useRealtimeUserCount] Realtime subscription closed');
        } else if (status === 'CHANNEL_ERROR') {
          console.warn('[useRealtimeUserCount] Realtime channel error');
        }
      });
